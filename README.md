...
- **v2.13**（2025/12/05）：
  - **核心優化**：移除爬蟲核心中的硬編碼規則（如特定 PM 站的驗證邏輯與排除清單），改為完全讀取設定檔，提升通用性與穩定性
  - **智能重置**：新增網域偵測機制，當切換不同網域爬取時，自動重置爬蟲狀態，避免資料混淆
  - **設定整合**：確保 GUI 設定完整傳遞至爬蟲核心，解決過度客製化導致新網站爬取中斷的問題
- **v2.14**（2025/12/05）：
  - **強化空頁面偵測**：重新實作 menu.php 空清單頁判斷邏輯
    - 從 breadcrumb 定位商品列表容器
...
- **v2.16**（2025/12/05）：
  - **可設定化空頁面偵測**：空頁面判斷邏輯不再寫死在程式碼中
    - 新增 `empty_page_detection` 設定區塊
    - 可自訂 URL 特徵、選擇器、分頁模式等
    - 支援不同網站的 DOM 結構調整
  - 更新規格書至 v2.0，包含完整設定說明
